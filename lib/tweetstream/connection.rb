...
module TweetStream
  ...
  #
  # This connection class is for use inside an EventMachine event
  # loop.  Just call one of the provided methods and provide a
  # block that will perform actions on a yielded TweetStream::Status.
  # For example:
  #
  #     TweetStream::Connection.new('user','pass').track('fail') do |status|
  #       puts "[#{status.user.screen_name}] #{status.text}"
  #     end
  #
  # If you don't need to control how your EventMachine event loop is
  # constructed, you can just use the TweetStream::Client class.
  class Connection
    ...
    def firehose(query_parameters = {}, &block)
      connect('statuses/firehose', query_parameters, &block)
    end
    ...
    def retweet(query_parameters = {}, &block)
      connect('statuses/retweet', query_parameters, &block)
    end
    ...
    def sample(query_parameters = {}, &block)
      connect('statuses/sample', query_parameters, &block)
    end
    ...
    def filter(query_params = {}, &block)
      ...
      connect('statuses/filter', query_params.merge(:method => :post), &block)
    end
    ...
    def site_follow(*user_ids, &block)
      ...
      connect('site', query_params.merge(:method => :post, :host => 'betastream.twitter.com', :version => '2b'), &block)
    end
    ...
    def connect(path, query_parameters = {}, &block) #:nodoc:
